...
def create_application(job_id:UUID, payload:ApplicationCreate, db:Session = db_dependency, current_user:User=cu_dependency):
...
def get_job_applications(job_id:UUID, application_status:ApplicationStatus | None=None, page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), db:Session=db_dependency, current_user:User=cu_dependency):
...
def get_my_applications(application_status:ApplicationStatus | None = None, page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), search: str | None = Query(None, min_length=1),db:Session=db_dependency, current_user:User=cu_dependency):
...
def update_application_Staus(application_id:UUID, payload:ApplicationStatusUpdate, db:Session=db_dependency, current_user:User=cu_dependency):
...
def withdraw_application(application_id:UUID, db:Session=db_dependency, current_user:User=cu_dependency):
